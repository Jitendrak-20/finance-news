...
function createBody(title, rawArticle) {
  ...
  return [
    `<p>${escapeHtml(rawArticle.raw_summary || "")}</p>`,
...
async function handleGenerateJob(state, options = {}) {
  const limit = Number.isInteger(options.limit) && options.limit > 0 ? options.limit : 2;
  const queue = getRawQueue(state).slice(0, limit);
  const built = [];
...
async function retryImageForArticle(article, state) {
  const previousImageUrl = article.image_url || "";
  ...
  return {
    article,
    image_url: nextImageUrl,
    image_updated: Boolean(nextImageUrl) && nextImageUrl !== previousImageUrl
  };
}

async function ensureRetriedImageReady(article, state, options = {}) {
  const attempts = Number.isInteger(options.attempts) && options.attempts > 0 ? options.attempts : 3;
  const delayMs = Number.isInteger(options.delayMs) && options.delayMs >= 0 ? options.delayMs : 1200;
  let lastResult = null;

  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    lastResult = await retryImageForArticle(article, state);
    if (lastResult.image_url) {
      return {
        article: lastResult.article,
        image_url: lastResult.image_url,
        image_updated: lastResult.image_updated,
  ...
  state.jobs.unshift(createJob("retry-image-wait", "partial", {
    article_id: article.id,
    attempts,
    last_image_url: lastResult && lastResult.image_url ? lastResult.image_url : ""
  }));
...
async function forcePublishArticle(article, state) {
...
function nextQuarterHourIso(from = new Date()) {
...
async function runPublishingPipeline(state, options = {}) {
  const startedAt = new Date();
  const existingDraftIds = new Set(state.articles.filter((item) => item.status === "draft").map((item) => item.id));
  const batchSize = Number.isInteger(options.batchSize) && options.batchSize > 0 ? options.batchSize : 1;

  await handleFetchJob(state);
  await handleGenerateJob(state, { limit: batchSize });

  const batchDrafts = state.articles.filter((item) => item.status === "draft" && !existingDraftIds.has(item.id));
  const publishedIds = [];

  for (const article of batchDrafts) {
    if (options.retryImages !== false) {
      const imageResult = await ensureRetriedImageReady(article, state);
      if (!imageResult.image_url) {
        state.jobs.unshift(createJob("pipeline-skip-publish", "partial", {
          article_id: article.id,
    ...
    if (options.forcePublish !== false) {
      await forcePublishArticle(article, state);
  ...
  const summary = {
    ...
    fetched_job_status: state.jobs.find((job) => job.job_type === "fetch" || job.job_type === "fetch-partial")?.status || "unknown",
    generated_count: batchDrafts.length,
    retried_image_count: options.retryImages === false ? 0 : batchDrafts.length,
    force_published_count: options.forcePublish === false ? 0 : publishedIds.length,
    published_article_ids: publishedIds,
    next_run_at: nextQuarterHourIso(new Date())
  };

  state.jobs.unshift(createJob("pipeline", "success", summary));
  return summary;
}
...
function requireAdmin(req, res) {
  ...
  sendJson(res, 401, { error: "Incorrect password or admin access is not configured." });
  return false;
}
...
async function handleApi(req, res, url) {
  ...
  if (req.method === "GET" && pathname === "/api/articles") {
    ...
    if (status === "draft" && !requireAdmin(req, res)) return;
  ...
  if (req.method === "POST" && pathname === "/api/jobs/pipeline") {
    if (!requireAdmin(req, res)) return;
    const summary = await runPublishingPipeline(state);
    await writeDb(state);
    return sendJson(res, 200, { ok: true, summary, jobs: state.jobs.slice(0, 8) });
  }
  ...
  if (req.method === "POST" && pathname === "/api/article-action") {
    ...
    if (action === "publish") {
      ...
      await retryImageForArticle(article, state);
      await writeDb(state);
      return sendJson(res, 200, hydrateArticle(article, state));
    } else {
